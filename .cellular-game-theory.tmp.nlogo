...
to game-theory
  ask passengers with [safe-passenger? = True]
  [
    if count passengers with [next-move = [next-move] of myself] > 1
    [
      let players passengers with [next-move = [next-move] of myself]
      let max-move max pos-final
      let dir position max-move pos-final
      let seed random 100
      let C count passengers with [next-move = [next-move] of myself and strategy = 0]
      let D count passengers with [next-move = [next-move] of myself and strategy = 1]
      let mean-pay-D get-mean-payoff 1
      let mean-pay-C get-mean-payoff 0
      let sum-other 0
      ask passengers with [next-move = [next-move] of myself and strategy = 0]
      [
        set sum-other sum-other + ks * exp(get-mean-payoff 0)
      ]
      ask passengers with [next-move = [next-move] of myself and strategy = 1]
      [
        set sum-other sum-other + ks * exp(get-mean-payoff 1)
      ]
      let pos-change-s 0
      ifelse strategy = 0
      [
        set pos-change-s (exp(ks * mean-pay-D) / (exp(ks * mean-pay-D)) + (exp(ks * payoff)))
      ]
      [
        set pos-change-s (exp(ks * mean-pay-C) / (exp(ks * mean-pay-C)) + (exp(ks * payoff)))
      ]
      if pos-change-s > 0.5
      [
        set strategy (abs 1 - strategy)
      ]
      let new-pos exp(ks * (get-mean-payoff strategy)) / sum-other
      set pos-final replace-item dir pos-final new-pos
      ifelse strategy = 0
      [
        ifelse D > 0
        [
          set payoff (payoff * (game-time + 1) + (1 - cost)) / (game-time + 2)
        ]
        [
          set payoff (payoff * (game-time + 1) + (1)) / (game-time + 2)
        ]
      ]
      [
        ifelse D > 0
        [
          set payoff (payoff * (game-time + 1)) / (game-time + 2)
        ]
        [
          set payoff (payoff * (game-time + 1) + (1 + cost)) / (game-time + 2)
        ]
    ...
    let max-value max pos-final
    set dir position max-value pos-final
    set inde dir
    set heading (dir * 45)
    set next-move patch (round (ycor + cos(heading))) (round (xcor + sin(heading)))
    ]
  ]
end
...
to move-passenger
  ...
  [
    if count passengers with [next-move = [next-move] of myself] > 1
    [
      if count passengers with [next-move = [next-move] of myself and mass > [mass] of myself] > 0
      [
        set next-move patch-here
      ]
    ]
    set xcor [pxcor] of next-move
    set ycor [pycor] of next-move
  ]
end

to-report get-mean-payoff [s]
  let mean-payoff 0
  ifelse s  = 0
  [
    ask passengers with [next-move = [next-move] of myself]
    [
      ifelse s = 0
      [
        set mean-payoff mean-payoff + 1
      ]
      [
        set mean-payoff mean-payoff + (1 - cost)
      ]
    ]
    set mean-payoff mean-payoff / (count passengers with [next-move = [next-move] of myself])
    report mean-payoff
  ]
  [
    ask passengers with [next-move = [next-move] of myself]
